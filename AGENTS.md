...
12. **未参照 `working-on-backend` skill 不得编辑或新增 `packages/backend/` 下的文件**
13. **未参照 `working-on-frontend` skill 不得编辑或新增 `packages/frontend/` 下的文件**
14. **未参照 `shipping-misskey-change` skill 不得 commit / 创建 PR / 把工作交还给用户**
