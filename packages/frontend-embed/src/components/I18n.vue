...
const slots = defineSlots<Record<string, () => VNodeChild>>();
...
const render = () => {
	const children: VNodeChild[] = parsed.value.map(x => typeof x === 'string' ? (props.textTag ? h(props.textTag, x) : x) : slots[x.arg]?.());
